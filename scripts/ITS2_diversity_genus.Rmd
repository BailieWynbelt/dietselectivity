...
no_millet_vouchers_ITS2_genus <- no_millet_vouchers_ITS2_genus %>% 
  filter((sample_type != "plant") %>% 
  replace_na(TRUE))
...
distinct_genus_10 <- under_10_reads_ITS2  %>% 
  select(Genus)
  distinct()
# = 
distinct_genus_25 <- under_25_reads_ITS2  %>% 
  select(Genus)
  distinct()
# = 
distinct_genus_50 <- under_50_reads_ITS2  %>% 
  select(Genus) %>%  
  distinct()
# = 
